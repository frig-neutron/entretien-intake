...
global.SpreadsheetApp = mockz<SpreadsheetApp>({
  getActive() {
    return mockz<Spreadsheet>({
...
function logSheet(): Sheet {
  ...
  type MyMockImpl<T> = DeepPartial<T> | (() => DeepPartial<T>)

  function mockz<T>(impl: MyMockImpl<T>, thiz: any = {}): T {
    if (typeof impl === "function") {
      return impl.bind(thiz).call() as T
    }
    return impl as T
  }
